...
def parse_children(line):
    quantities, colors = [], []
    words = line.split(" contain ")[1].split(" ")

    for i, word in enumerate(words):
        if word.isdigit():
            quantities.append(int(word))
            colors.append(" ".join(words[i+1: i+3]))
    
    return list(zip(quantities, colors))


with open("day7_input.txt", "r") as file:
    # create dictionary that stores parents and children of each bag
    for line in file.read().split("\n"):
        words = line.split(" ")
        color = " ".join(words[:2])
        children = parse_children(line)

        if color in bags:
            bags[color]['children'] = children
        else:
            bags[color] = {'children': children, 'parents': set()}

        for n, c in children:
            if c not in bags:
                bags[c] = {'parents': set()}
            bags[c]['parents'].add(color)


def count_children(color):
    count = 0
    for n, c in bags[color]['children']:
        count += n * (count_children(c) + 1)
    return count

 
def count_parents(color):
    def recursive_search(c):
        parents = set()
        for p in bags[c]['parents']:
            parents.add(p)
            parents.update(recursive_search(p))
        return parents
    return len(recursive_search(color))


print("Part 1 solution:", count_parents("shiny gold"))
print("Part 2 solution:", count_children("shiny gold"))