...
# passport control. You write a quick program to use your phone's camera to
# scan all of the nearby boarding passes (your puzzle input); perhaps you can
# find your seat through process of elimination. Instead of zones or groups,
...
# "left", and R means "right". The first 7 characters will either be F or B;
# these specify exactly one of the 128 rows on the plane (numbered 0 through
...
bit_mapping = {'F': '0', 'B': '1', 'L': "0", 'R': "1"}


def row(bp):
    bits = "".join([bit_mapping[x] for x in bp[:7]])
...
def ids(arr):
    return  [(row(x) * 8 + col(x)) for x in arr]


bp_ids = ids(bps)

print("Part 1 solution:", max(bp_ids))
...
# be missing from your list as well. Your seat wasn't at the very front or
# back, though; the seats with IDs +1 and -1 from yours will be in your list.
# What is the ID of your seat?


def my_seat(id_list):
    s = sorted(id_list)
    for index, _id in enumerate(s[:-1]):
        if _id != s[index+1] - 1:
            return _id + 1


print("Part 2 solution:", my_seat(bp_ids))