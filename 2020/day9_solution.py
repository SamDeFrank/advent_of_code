...
# screen in the seat in front of you. Though the port is non-standard, you manage to connect it to your computer through
# the clever use of several paperclips. Upon connection, the port outputs a series of numbers (your puzzle input). The
# data appears to be encrypted with the eXchange-Masking Addition System (XMAS) which, conveniently for you, is an old
...
# only number that does not follow this rule is 127. The first step of attacking the weakness in the XMAS data is to
...
with open("day9_input.txt", "r") as file:
  output = [int(x) for x in file.read().split("\n")]
...
for i, num in enumerate(output[25:]):
  if not validate(output[i:i+25], num):
    p1_solution = num
    print(p1_solution)
    break
...
while back < len(output):
  section = output[back:front]
  list_sum = reduce(lambda x,y: x+y, section)
  if list_sum < p1_solution:
    front += 1
  elif list_sum == p1_solution:
    section.sort()
    print("Part 2 solution: ",section[0] + section[-1])
    break
  else:
    back += 1